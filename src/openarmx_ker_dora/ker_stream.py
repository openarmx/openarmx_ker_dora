...
class KERStream:
    def __init__(self, transport: str = 'usb', port: str = '/dev/ttyACM0',
        ...
        self._usb_kernel_driver_detached = False
        self._ep_in = None
        self._ep_out = None
        self._serial = None
    ...
    def _connect_usb(self) -> None:
        ...
        for candidate in configuration:
            ...
            candidate_in = usb.util.find_descriptor(
                candidate, custom_match=lambda endpoint: usb.util.endpoint_direction(
        ...
        try:
            if device.is_kernel_driver_active(interface_number):
                ...
                self._usb_kernel_driver_detached = True
        except NotImplementedError:
            pass
        usb.util.claim_interface(device, interface_number)

        self._device = device
        self._usb_interface = interface_number
    ...
    def close(self) -> None:
        ...
        if self._device:
            import usb.core
            import usb.util
            if self._usb_interface is not None:
                try:
                    usb.util.release_interface(self._device, self._usb_interface)
                except usb.core.USBError:
                    pass
                if self._usb_kernel_driver_detached:
                    try:
                        self._device.attach_kernel_driver(self._usb_interface)
